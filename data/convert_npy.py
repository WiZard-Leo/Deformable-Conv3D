...
def process_file(file_path):
    ...
    nii_data = interpolate_cube(nii_data).astype(np.float32)    
    # 将数据保存为 .npy 文件，文件名保持一致，但扩展名变为 .npy
    # print(os.path.splitext(file_path)[0][:-4])        
    npy_file_path = os.path.splitext(file_path)[0][:-4] + '.npy'        
    npy_file_path = os.path.join('/opt/data/private/dataset/ctdata_npy', npy_file_path.split('/')[-1])
...
if __name__ == "__main__":
    ...
    folder_path = "/opt/data/private/dataset/ctdata"
